...
def generate_flashcards(text: str, count: int = 2) -> List[Dict[str, str]]:
    ...
    prompt = (
        f"Generate exactly 2 flashcards in JSON format from the transcript chunk below.\n"
        "Rules:\n"
...
def generate_flashcards_from_docs(max_chunks: int = 5, cards_per_chunk: int = 2) -> List[Dict[str, str]]:
    docs = get_chunks()
    ...
    for i, doc in enumerate(docs):
        ...
        print(f"[OK] chunk {i+1}/{len(docs)} -> {len(cards)} cards")
